...
def main():
    ...
    # load model
    simple_vlm = SimVLMModel.from_specific_checkpoint(checkpoint_path=conf.evaluate.model.check_path, llm_config=llm_config, visual_config=visual_config, multi_model_config=multi_model_config)

    images_path = conf.evaluate.data.image_path
    prompt = "描述一下图中的内容，精炼而不失细节。"
    ...
    formatted_input = tokenizer.apply_chat_template(conversations, tokenize=False, add_generic_prompt=True)
    ...
    for image in os.listdir(images_path):
        ...
        with torch.no_grad():
            outputs = simple_vlm.generate(
                ...
                max_length=conf.evaluate.generation.max_length,
